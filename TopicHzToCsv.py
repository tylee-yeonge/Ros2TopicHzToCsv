...
class TopicHzToCsv(Node):
    def __init__(self):
        super().__init__(
            f"topic_hz_to_csv_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S%f')}"
        )
        ...
        self.reliability_str = (
            self.get_parameter("reliability").get_parameter_value().string_value
        )  # 추가: reliability 값 가져오기

        # 추가: reliability 문자열을 ReliabilityPolicy로 변환
        if self.reliability_str.lower() == "reliable":
            self.reliability_policy = ReliabilityPolicy.RELIABLE
        elif self.reliability_str.lower() == "best_effort":
            self.reliability_policy = ReliabilityPolicy.BEST_EFFORT
        else:
            self.get_logger().warn(
                f"Invalid reliability value: {self.reliability_str}. Using BEST_EFFORT as default."
            )
            self.reliability_policy = ReliabilityPolicy.BEST_EFFORT
        ...
        try:
            ...
            if len(module_parts) == 3:
                ...
                self.get_logger().info(
                    f"Successfully imported message type: {self.msg_type.__name__} from {package_name}.{module_name}"
                )
            else:
                self.get_logger().error(
                    f"Invalid message type format: {self.message_type_str}. Expected 'pkg/module/Type'."
                )
                raise ImportError("Invalid message type format")
        except (ImportError, AttributeError, IndexError) as e:
            self.get_logger().error(
                f"Could not import message type '{self.message_type_str}': {e}"
            )
            self.msg_type = None
        ...
        if not file_exists or is_empty:
            ...
            self.get_logger().info(
                f"Created new CSV file: {self.csv_filename} with header."
            )
        else:
            self.get_logger().info(
                f"Appending to existing CSV file: {self.csv_filename}"
            )

        if self.msg_type:
            qos_profile = QoSProfile(
                reliability=self.reliability_policy,  # 변경: 파라미터로 받은 값 사용
            ...
            self.get_logger().info(
                f"Subscribing to topic: {self.topic_name} with type {self.message_type_str}"
            )
            self.get_logger().info(f"Saving data to: {self.csv_filename}")
            self.get_logger().info(
                f"Frequency calculation window size: {self.window_size} messages"
            )
            ...
            self.get_logger().error(
                "Subscription not created due to message type import failure."
            )
    ...
    def save_data_callback(self):
        ...
        self.get_logger().info(
            f"Timestamp (Local): {formatted_ros_timestamp}, Hz: {hz_rounded_int}",
...
def main(args=None):
    ...
    if topic_hz_to_csv_node.msg_type is None:
        topic_hz_to_csv_node.get_logger().fatal(
            "Failed to initialize node due to message type error. Shutting down."
        )
    else:
        try:
            ...
            topic_hz_to_csv_node.get_logger().error(
                f"An error occurred during spin: {e}"
            )
